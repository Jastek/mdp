...
router.get('/positions', async ( req: any, res) => {
    try {
        ...
        const count = await Position.count(search);
        const data = await Position.find(search).sort(order).limit(limit).skip(offset);
 ...
 * List all paginated
 */
 router.get('/positions/all', async ( req: any, res) => {
    try {
 ...
 * Get by id
 */
router.get('/positions/:id', async ( req: any, res) => {
    try {
        if (!req.user.isAdmin) return res.status(401).send('Not Authorized');
 ...
 */
 router.get('/positions/:id/people', async ( req: any, res) => {
    try {
        const id = req.params.id;
 ...
 * Create
 */
router.post('/positions', async (req: any, res) => {
    try {
        if (!req.user.isAdmin) return res.status(401).send('Not Authorized');
 ...
 * Update
 */
 router.patch('/positions/:id', async (req: any, res) => {
    try {
        if (!req.user.isAdmin) return res.status(401).send('Not Authorized');
        ...
        const result = await Position.findByIdAndUpdate(id, updateData);
...
router.delete('/positions/:id', async (req:any, res) => {
    try {
        if (!req.user.isAdmin) return res.status(401).send('Not Authorized');
        ...
        const person = await Position.findByIdAndDelete(id);
